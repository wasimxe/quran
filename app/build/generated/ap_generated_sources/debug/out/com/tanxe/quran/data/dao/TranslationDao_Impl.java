...
public final class TranslationDao_Impl implements TranslationDao {
  ...
  public TranslationDao_Impl(@NonNull final RoomDatabase __db) {
    ...
    this.__insertionAdapterOfTranslation = new EntityInsertionAdapter<Translation>(__db) {
      ...
      protected String createQuery() {
        return "INSERT OR REPLACE INTO `translations` (`id`,`surahNumber`,`ayahNumber`,`text`,`edition`,`language`) VALUES (nullif(?, 0),?,?,?,?,?)";
      }
    ...
    this.__preparedStmtOfDeleteEdition = new SharedSQLiteStatement(__db) {
      ...
      public String createQuery() {
        final String _query = "DELETE FROM translations WHERE edition = ?";
        return _query;
      }
    };
  }
  ...
  @Override
  public void insertAll(final List<Translation> translations) {
    __db.assertNotSuspendingTransaction();
    __db.beginTransaction();
    try {
      __insertionAdapterOfTranslation.insert(translations);
  ...
  @Override
  public Translation getTranslation(final int surah, final int ayah, final String edition) {
    final String _sql = "SELECT * FROM translations WHERE surahNumber = ? AND ayahNumber = ? AND edition = ? LIMIT 1";
  ...
  public List<String> getEditionsByLanguage(final String language) {
    final String _sql = "SELECT DISTINCT edition FROM translations WHERE language = ?";
  ...
  @Override
  public List<Translation> searchInEdition(final String edition, final String query) {
    final String _sql = "SELECT * FROM translations WHERE edition = ? AND (text LIKE '%' || ? || '%') ORDER BY surahNumber, ayahNumber";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 2);
    int _argIndex = 1;
    if (edition == null) {
      _statement.bindNull(_argIndex);
